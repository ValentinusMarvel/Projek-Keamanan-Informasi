...
# Apply StandardScaler to match baseline LSTM normalization
from sklearn.preprocessing import StandardScaler
scaler = StandardScaler()
train_flat = X_train_full.reshape(-1, X_train_full.shape[-1])
scaler.fit(train_flat)
X_train_full = scaler.transform(X_train_full.reshape(-1, X_train_full.shape[-1])).reshape(X_train_full.shape)
X_test_global = scaler.transform(X_test_global.reshape(-1, X_test_global.shape[-1])).reshape(X_test_global.shape)
...
# Load pre-trained baseline weights to kickstart the FL process with transfer learning!
baseline_path = PROJECT_ROOT / 'outputs' / 'models' / 'baseline_lstm.pt'
if baseline_path.exists():
    print("Loading Pre-trained Baseline LSTM weights to kickstart FL...")
    checkpoint = torch.load(baseline_path, map_location=device)
    global_model.load_state_dict(checkpoint['state_dict'], strict=True)
else:
    print("WARNING: baseline_lstm.pt not found. Running from scratch.")
...
def server_evaluate(server_round: int, parameters, config):
    ...
    temp_res_path = PROJECT_ROOT / 'outputs' / 'reports' / f'temp_round_transfer_{server_round}.json'
    temp_res_path.write_text(json.dumps({"round": server_round, "global_loss": float(loss), "global_accuracy": float(acc)}))
    return float(loss), {"accuracy": float(acc)}

def start_server_fn():
    print("Starting Advanced Transfer FL Server...")
...
def start_client_fn(cid: int):
    print(f"Starting Transfer FL Client {cid}...")
    import time
    max_retries = 15
...
def main():
    if "--server" in sys.argv:
        ...
        print("Orchestrator: Starting Advanced Transfer FL Server Process...")
        server_proc = subprocess.Popen([sys.executable, __file__, "--server"])
        
        # Give server a brief moment to start socket listener
        time.sleep(5.0)

        print("Orchestrator: Starting Advanced Transfer FL Client Processes...")
        client_procs = []
        ...
        for r in range(1, rounds + 1):
            temp_path = PROJECT_ROOT / 'outputs' / 'reports' / f'temp_round_transfer_{r}.json'
            if temp_path.exists():
        ...
        payload = {
            ...
            'initialized_with_pretrained': True
        }
        
        out_path = PROJECT_ROOT / 'outputs' / 'reports' / 'fl_transfer_metrics.json'
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(payload, indent=2), encoding='utf-8')
        print(f"Saved transfer learning FL metrics to {out_path}")

        model_path = PROJECT_ROOT / 'outputs' / 'models' / 'fl_transfer_lstm.pt'
        torch.save({'state_dict': global_model.state_dict(), 'label_to_idx': label_to_idx}, model_path)
        print(f"Saved global Transfer FL model to {model_path}")
