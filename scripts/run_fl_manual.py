...
def server_evaluate(server_round: int, parameters, config):
    # In Flower 1.x Strategy.evaluate, parameters is already a list of ndarrays (NDArrays)
    ndarrays = parameters
    ...
    # Write temp round results to disk so the orchestrator process can read it
    temp_res_path = PROJECT_ROOT / 'outputs' / 'reports' / f'temp_round_{server_round}.json'
...
def start_client_fn(cid: int):
    print(f"Starting FL Client {cid}...")
    import time
    max_retries = 15
...
def main():
    if "--server" in sys.argv:
        ...
        for r in range(1, rounds + 1):
            temp_path = PROJECT_ROOT / 'outputs' / 'reports' / f'temp_round_{r}.json'
            if temp_path.exists():
        ...
        out_path = PROJECT_ROOT / 'outputs' / 'reports' / 'fl_metrics.json'
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(payload, indent=2), encoding='utf-8')
        print(f"Saved federated learning metrics to {out_path}")
