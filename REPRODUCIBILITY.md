...
Dokumen ini disusun sebagai panduan langkah demi langkah bagi kontributor dan anggota kelompok untuk mereproduksi seluruh metrik, model, dan grafik visualisasi pada proyek Keamanan Informasi ini secara konsisten dan bebas dari kendala teknis (seperti Jupyter freeze atau port lock).

---

## 1. Persiapan Lingkungan dan Dependensi

Proyek ini dikembangkan menggunakan Python 3.10+ (direkomendasikan Python 3.11 atau 3.12) pada sistem operasi Windows.

### Langkah A: Setup Virtual Environment (.venv)
Buka PowerShell atau Terminal di direktori root proyek ini, kemudian jalankan:
```powershell
...
## 2. Langkah Reproduksi Pipeline (Step-by-Step)

Ada dua cara untuk mereproduksi keluaran proyek ini. Cara terbaik dan paling stabil untuk menghindari kelambatan atau hang pada editor Jupyter adalah menggunakan Terminal VS Code untuk bagian pelatihan berat atau simulasi jaringan, kemudian menggunakan Jupyter Notebook untuk memvisualisasikan hasilnya.

### OPSI A: Eksekusi Cepat via Terminal (Sangat Direkomendasikan dan Stabil)
Jalankan perintah-perintah berikut di Terminal VS Code Anda secara berurutan. Metode ini menjamin kestabilan proses dan bebas dari penyumbatan buffer konsol editor:
...
# Langkah 3: Melatih Baseline LSTM terpusat (Akurasi 63.29%)
...
# Langkah 6: Menjalankan Simulasi FL + DP (Flower dan Opacus: Port 8090)
...
# Langkah 12: Menjalankan Advanced Federated Transfer Learning (Akurasi melompat ke 63.29% pada Non-IID: Port 8092)
.venv\Scripts\python.exe scripts/run_fl_transfer.py

# Langkah 13: Compile Seluruh Notebook Hasil dan Dashboard Laporan Akhir
...
> * **Penyebab**: Terlalu banyak kernel Python (`ipykernel_launcher`) yang menggantung aktif di latar belakang sistem Windows Anda, memicu DLL atau file lock deadlock pada berkas PyTorch/Pandas.
...
* **`fl_transfer_lstm.pt`**: Checkpoint model global FL dengan Transfer Learning skala Z-score (Akurasi melompat kembali ke **63.29%**!).
...
* **`privacy_utility_tradeoff.png`**: Kurva trade-off akurasi dan EER vs Epsilon.
...
* **`final_summary_table.csv`** dan **`final_summary_bundle.json`**: Gabungan seluruh metrik komparatif akhir proyek Anda.
